...
(function(){
  ...
  window.svg = function(m){
    try {
      ...
      if(M.f=='SISIR'){
        n=v===0?8:v===1?6:5;
        for(i=0;i<n;i++){var y=v===0?22+i*14:v===1?28+i*16:30+i*20;g+='<path d="M18 '+y+' h24" '+W+'/>';}
      }
      if(M.f=='BABI'){
    ...
    } catch(e){console.error('svg err',e);return '<svg viewBox="0 0 60 140"><rect x="1" y="1" width="58" height="138" rx="6" fill="#f5f0e6"/></svg>';}
  };
  ...
  function win12(c){var t=0,i;for(i=0;i<30;i++)t+=c[i];if(t!==12)return false;
    for(var p=0;p<30;p++){if(c[p]<2)continue;var tr=[];for(var m=0;m<30;m++){if(c[m]-(m===p?2:0)>=3)tr.push(m);}
      for(var a=0;a<tr.length;a++)for(var b=a+1;b<tr.length;b++)for(var d=b+1;d<tr.length;d++){
        var cc=c.slice();cc[p]-=2;cc[tr[a]]-=3;cc[tr[b]]-=3;cc[tr[d]]-=3;var ok=true,s=0;
        for(i=0;i<30;i++){if(cc[i]<0)ok=false;s+=cc[i];}if(ok&&s===1)return true;}}return false;}
    ...
    if(IS_HOST){
      ...
      if(p.ev==='state'){
        ...
        if(over){
          ...
          if(ovT)ovT.innerText=p.ovT;if(ovM)ovM.innerText=p.ovM;if(ov)ov.style.display='flex';
        } else {var ov=$('ov');if(ov)ov.style.display='none';}
        render();
      }
    }
  }
  ...
  function drawLobby(){
    var el=$('seatList');if(!el)return;
    var h='';SEATS.forEach(function(s){h+='<span class="chip">'+(s.bot?'BOT:':'')+' '+(s.n||'kosong')+'</span> ';});
    h+='<div class="dim" style="margin-top:6px">'+(IS_HOST?'Kamu HOST - biarkan halaman TERBUKA.':'Tamu - menunggu host memulai...')+'</div>';
    el.innerHTML=h;
  ...
  function render(){
    ...
    for(var p=0;p<4;p++){
      ...
      if(sEl)sEl.innerText=String.fromCharCode(9733).repeat(wins[p]);
      if(aEl)aEl.innerText=SEATS[p].bot?'BOT':'';
    ...
    if(my)stat(fase==='draw'?'Giliranmu: CABUT (atau MAKAN)':'Tangan 12: KETUK kartu lalu TURUN');
  ...
  function startRound(){
    ...
    started=true;
    var lobby=$('lobby'),game=$('gameArea');
    if(lobby)lobby.style.display='none';if(game)game.style.display='block';
  ...
  function afterMyDiscard(s){
    var r=rid;
    for(var b=0;b<4;b++){if(b===s)continue;if(SEATS[b].bot&&cnt(hands[b])[last.card]>=2&&Math.random()<0.5){
      setTimeout(function(){if(r!==rid)return;hands[b].push(last.card);disc[last.from].pop();last=null;stat(SEATS[b].n+' MAKAN kartu!');botFinish(b);},700);return;}}
  ...
  function botFinish(i){
    ...
    setTimeout(function(){if(r!==rid)return;
      if(!SEATS[MY].bot&&cnt(hands[MY])[best]>=2&&turn===MY){showMakan(true);stat(SEATS[i].n+' buang '+MID[best].f+' - MAKAN?');mkT=setTimeout(function(){showMakan(false);next();},2500);}
      else next();},400);
  }
  function endRound(w,why){
    ...
    var msg=(done?'JUARA: '+SEATS[w].n+' | ':'')+'Skor: '+wins.join(' - ');
  ...
  console.log('Room:',code);
    ...
    } catch(e){console.error('bStart',e);}
    ...
    try {
      ...
      if(bJoin){bJoin.onclick=function(){
        ...
        drawLobby();
        setTimeout(function(){if(!seated&&!started){var rl=$('roomLink');if(rl)rl.innerText='Host belum terdeteksi - pastikan host membuka halamannya.';}},8000);
      };console.log('bJoin ok');}
    } catch(e){console.error('bJoin',e);}
    ...
    } catch(e){console.error('Game buttons',e);}
